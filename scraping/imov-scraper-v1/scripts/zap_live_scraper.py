...
BASE_URLS = [
    ("geral", f"{ZAP}/venda/imoveis/ce+fortaleza/"),
    ("apartamentos", f"{ZAP}/venda/apartamentos/ce+fortaleza/"),
    ("casas", f"{ZAP}/venda/casas/ce+fortaleza/"),
]
ZAP_NEIGHBORHOOD_SLUGS = [
    "aldeota",
    "benfica",
    "cambeba",
    "centro",
...
def known_neighborhood_slugs() -> set[str]:
    ...
    slugs.update(ZAP_NEIGHBORHOOD_SLUGS)
    slugs.discard("")
    return slugs
...
def normalize_row(listing: dict, ld_item: dict) -> dict:
    ...
    return {
        ...
        "andar": int_cell(amenities.get("unitFloor") if amenities.get("unitFloor") is not None else features.get("andar")),
        ...
        "tem_varanda_gourmet": bool_cell(features.get("varanda_gourmet")),
...
def query_urls() -> list[tuple[str, str, int]]:
    ...
    seen_slugs = set()
    for slug in ZAP_NEIGHBORHOOD_SLUGS:
        seen_slugs.add(slug)
        urls.append((f"bairro:{slug}", f"{ZAP}/venda/imoveis/ce+fortaleza++{slug}/", 35))
    for bairro in sorted(BAIRROS_FORTALEZA):
        slug = slugify(bairro)
        if not slug or slug in seen_slugs:
            continue
        seen_slugs.add(slug)
        urls.append((f"bairro:{bairro}", f"{ZAP}/venda/imoveis/ce+fortaleza++{slug}/", 25))
    return urls
...
def collect(limit: int, delay: float):
    ...
    for label, base_url, max_pages in query_urls():
        ...
        for page in range(1, max_pages + 1):
            ...
            print(f"[ZAP] {label} p{page}: {len(rows)}/{limit}", flush=True)
            try:
                html_text = fetch(url)
            except Exception as exc:
                print(f"[ZAP] parada em {label} p{page}: {exc}", flush=True)
                break
            added = 0
            ...
            for row in parsed:
                ...
                if len(rows) >= limit:
                    return rows
            print(f"[ZAP] {label} p{page}: {added} novos", flush=True)
            if added == 0:
                empty_streak += 1
                if empty_streak >= 3:
                    break
...
def main():
    ap = argparse.ArgumentParser(description="Coleta ZAP live para venda de apartamentos/casas em Fortaleza/CE.")
    ap.add_argument("--out", default=str(ROOT / "saida" / "zap_live_20k.csv"))
